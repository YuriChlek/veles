...
const rows = [
    createData("English", "en", true, false),
    createData("German", "de", false, true),
    createData("France", "fr", false, false),
];
        ...
        <>
            ...
            <TableContainer sx={{borderRadius: 0, boxShadow: 0, border: "1px solid #0000001E"}} component={Paper}>
                <Table size="small" aria-label="a dense table">
                    <TableHead>
                        <TableRow>
                            <TableCell className={styles["admin-languages-header"]}
                                       align="center">{_t("Store View")}</TableCell>
                            <TableCell className={styles["admin-languages-header"]}
                                       align="center">{_t("Language Code")}</TableCell>
                            <TableCell className={styles["admin-languages-header"]}
                                       align="center">{_t("Default Frontend")}</TableCell>
                            <TableCell className={styles["admin-languages-header"]}
                                       align="center">{_t("Default Admin")}</TableCell>
                            <TableCell className={styles["admin-languages-header"]}
                                       align="center">{_t("Action")}</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {rows.map((row) => (
                            ...
                            >
                                <TableCell className={styles["admin-languages-cell"]} align="center" component="th"
                                           scope="row">
                                    {_t(row.language)}
                                </TableCell>
                                <TableCell className={styles["admin-languages-cell"]}
                                           align="center">{row.languageCode}</TableCell>
                                <TableCell className={styles["admin-languages-cell"]}
                                           align="center">{row.isDefaultFrontend ? _t("Yes") : _t("No")}</TableCell>
                                <TableCell className={styles["admin-languages-cell"]}
                                           align="center">{row.isDefaultAdmin ? _t("Yes") : _t("No")}</TableCell>
